...
def build_instructions() -> str:
    ...
    parts.extend([
        ...
        "",
        "  PRE-FILTER SQL:",
        "    Any SQL whose first column returns chunk IDs (messages.id).",
        "    Use the messages view — it has every column you need.",
        ...
        "    diverse             MMR diversity — spreads across subtopics, use for broad discovery",
        ...
        "RECIPES:",
        "",
        "  Nearest neighbors (DEFAULT — use this most of the time):",
        "    SELECT v.id, v.score, m.content",
...
def _machine_id() -> str | None:
    """Read ~/.flex/machine_id — generated by flex init. 8-char hex."""
    p = Path.home() / ".flex" / "machine_id"
