...
def build_instructions() -> str:
    ...
    parts.extend([
        ...
        "",
        "    'SELECT chunk_id FROM _types_message WHERE type = ''user_prompt'''",
        "",
        "    Compound:",
        ...
        "     WHERE t.type = ''user_prompt'' AND g.community_id = 3'",
        "",
        ...
        "      'SELECT chunk_id FROM _types_message WHERE type = ''user_prompt''') v",
        "    JOIN messages m ON v.id = m.id",
        "    LIMIT 10",
        "",
...
async def handle_list_tools() -> list[types.Tool]:
    """Return the flex tool with dynamic description and schema."""
    return [
        types.Tool(
            name="flex",
