...
def build_instructions() -> str:
    ...
    parts.extend([
        ...
        "  Use the messages view. Filter by type, session, tool, date, or any combination.",
        "",
        ...
        "  'SELECT id FROM messages WHERE type = ''user_prompt''",
        "   AND session_id IN (SELECT session_id FROM sessions WHERE community_id = 3)'",
        "",
        "VECTOR OPERATIONS",
        "  Semantic ranking over the filtered landscape. Returns (id, score) pairs.",
        "  vec_ops('_raw_chunks', 'query_text', 'tokens', 'pre_filter_sql')",
        "  Tokens are optional and compose freely — omit for raw cosine similarity.",
        "",
        "  diverse             MMR — spreads results across subtopics",
        ...
        "",
        "SQL COMPOSITION",
        "  Join vec_ops results back to messages/sessions to add context and shape output.",
        "  Group by session, filter by file, re-order by centrality, paginate — all here.",
        "",
        ...
        "",
        "  Exact term (domain, filename, error, UUID — FTS5):",
        ...
        "    -- Note: wrap terms with special chars in double-quotes: MATCH '\"axp.systems\"'",
        "",
        "  Nearest neighbors:",
        "    SELECT v.id, v.score, m.content",
        ...
        "    LIMIT 10",
        "",
        "  Broad discovery (diverse — survey a topic across subtopics):",
        ...
        "    LIMIT 5",
        "",
        "  Exact term + semantic context (FTS5 → vec_ops):",
        "    SELECT v.id, v.score, m.content",
        "    FROM vec_ops('_raw_chunks', 'YOUR TOPIC', '',",
        "      'SELECT c.id FROM chunks_fts JOIN _raw_chunks c ON chunks_fts.rowid = c.rowid",
        "       WHERE chunks_fts MATCH ''term''') v",
        "    JOIN messages m ON v.id = m.id",
        "    ORDER BY v.score DESC LIMIT 10",
        "",
        "METHODOLOGY: Query as Exploration",
        "",
        "  Start with @orient. Every cell describes itself — schema, presets, column notes.",
        "",
        "  Feel the data before writing complex queries.",
        "    What dates exist? What projects? How many sessions?",
        "    GROUP BY / COUNT(*) / SELECT DISTINCT tells you the shape. Free — no embeddings.",
        "",
        "  Discover then narrow. Start broad, let results redirect.",
        "    Broad vec_ops → discover themes → pre-filter the next query with what you found.",
        "    Push known constraints (date, session, type) into vec_ops 4th argument, not WHERE.",
        "",
        "  Pivot when the mode shifts.",
        "    Found a theme? Count it. Group it. Quantify.",
        "    Found an ID? Switch to exact retrieval — JOIN sections ORDER BY position.",
        "",
        "  Cross-cell when one cell isn't enough. Cells have different temporal coverage.",
        "    Check date ranges before assuming a cell has recent data.",
        "",
