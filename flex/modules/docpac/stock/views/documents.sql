...
SELECT
    ...
    src.temporal,
    src.doc_type,
    CASE WHEN INSTR(src.doc_type, '.') > 0
         THEN SUBSTR(src.doc_type, 1, INSTR(src.doc_type, '.') - 1) END AS category,
    CASE WHEN INSTR(src.doc_type, '.') > 0
         THEN SUBSTR(src.doc_type, INSTR(src.doc_type, '.') + 1)
         ELSE src.doc_type END AS subtype,
