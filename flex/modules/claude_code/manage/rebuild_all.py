...
# Selective pooling + mean centering shifted median pairwise similarity
# from 0.61 to -0.006. 0.55 threshold gives 15.2% density, ~48 communities.
...
def reembed_sources(db):
    ...
    Selective pooling: mean-pool of ALL chunks converges every session
    ...
    Chunks near the corpus centroid (boilerplate preambles, hook injections, generic
...
def rebuild_community_labels(db):
    ...
    # Dot-projects (e.g. .claude) are structural infrastructure that appears
...
def rebuild_repo_project(db):
    ...
    # re-evaluated by the full tier stack.
    infra_like = ' OR '.join(
