...
def _ingest_file_body(conn: sqlite3.Connection, parent_chunk_id: str,
    ...
    if existing:
        ...
        conn.execute(
            "DELETE FROM _raw_chunks WHERE id LIKE ?",
            (f"{old_parent}:fb:%",))
        conn.execute(
            "DELETE FROM _edges_source WHERE chunk_id LIKE ?",
            (f"{old_parent}:fb:%",))
        conn.execute(
            "DELETE FROM _types_file_body WHERE chunk_id LIKE ?",
            (f"{old_parent}:fb:%",))
    ...
    # Update index
    conn.execute(
        "INSERT OR REPLACE INTO _file_body_index VALUES (?,?,?,?,?)",
                ...
                for filepath, info in backups.items():
                    ...
                    if backup_path.exists():
                        try:
                            ...
                            _store_content_raw(conn, chunk_id, text, '_file_backup', ts_int)
                        except Exception:
                            pass
                if file_hashes:
                    ...
                    if tool_name in ('Read', 'Edit') and raw:
                        ...
                        if result_chunk:
                            ...
                            if target_file and len(raw) > 50:
                                ...
                                fb_items.append((chunk_id, target_file, clean, ts_int))
